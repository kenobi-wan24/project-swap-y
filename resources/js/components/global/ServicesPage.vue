        ...
        <div v-for="item in featuredItems" :key="item.id" class="swapy-card hscroll-card">
          <a :href="'/item/'+item.id" class="card-link">
            <div class="card-img-wrap">
              ...
              <!-- Featured: FEATURED badge + Match pill side by side -->
              <div class="pills-row">
...
/* ── Pills row (Featured cards: badge + match side by side) ── */
.pills-row {
  position: absolute;
  top: 10px;
  left: 10px;
...
/* Solo match pill (non-featured cards) — absolutely positioned */
.match-pill--solo {
  position: absolute;
  top: 10px;
  left: 10px;

...
const postedService = ref(null)
const stepLabels    = ['Portfolio Photos', 'Service Details', 'Description & Highlights']
...
const category  = ref('')
const rate       = ref('')
const rateType   = ref('Per project')
const delivery   = ref('Both')
const location   = ref('')
...
const categories = ['Design & Creative', 'Tech & Digital', 'Education & Tutoring', 'Home & Repair', 'Business', 'Creative']
const rateTypes  = ['Per hour', 'Per project', 'Per session', 'Per day']
const deliveryOptions = ['Remote', 'In-person', 'Both']

const step2Valid = computed(() => title.value.trim() && category.value)
...
async function initMap() {
  ...
  leafletMap = window.L.map('service-map', { center: [12.8797, 121.7740], zoom: 6, zoomControl: true })
...
// ── step 3: description, highlights, swap ─────────────────────────────────────
const description = ref('')
const swapFor     = ref('')
const tags        = ref([])
const tagOptions  = [
  'Fast delivery', 'Revisions included', 'Beginner-friendly', 'Portfolio available',
  'Certified', '5+ years experience', 'Flexible schedule', 'Same-day available',
]
...
const isDirty = computed(() =>
  images.value.length > 0 || title.value.trim() || category.value || rate.value ||
  location.value || description.value.trim() || swapFor.value.trim() || tags.value.length > 0
)
...
async function publishService() {
  ...
  formData.append('rate_type',   rate.value ? rateType.value : '')
  formData.append('delivery',    delivery.value)
  formData.append('swap_for',    swapFor.value)
  ...
  try {
    const res = await fetch('/services', { method: 'POST', headers: { 'X-CSRF-TOKEN': csrfToken }, body: formData })
    const data = await res.json()
    if (data.success) { postedService.value = data.service; posted.value = true }
  } catch (e) { console.error('Publish service failed', e) }
...
<div style="min-height:100vh;background:#fff;font-family:'DM Sans',sans-serif;">
  ...
  <div v-if="posted" style="min-height:100vh;display:flex;align-items:center;justify-content:center;padding:24px;">
    <div style="background:#fff;border-radius:24px;padding:48px 32px;max-width:480px;width:100%;text-align:center;box-shadow:0 4px 24px rgba(0,0,0,0.08);border:1px solid #EDE8E0;">
      ...
      <h2 style="font-size:1.5rem;font-weight:800;color:#3A3330;margin:0 0 10px;">Service Listed!</h2>
      <p style="font-size:0.9rem;color:#6b7280;margin:0 0 32px;">
        <strong style="color:#3A3330;">{{ postedService?.title }}</strong> is now live and ready for swap matches.
      </p>

      <div v-if="imagePreviews.length" style="width:100%;height:160px;border-radius:16px;overflow:hidden;margin-bottom:28px;">
        <img :src="imagePreviews[primaryIndex]" alt="Your service cover photo" style="width:100%;height:100%;object-fit:cover;">
      </div>

      <a href="/services"  style="display:block;width:100%;padding:14px;background:#ED730C;color:#fff;font-size:0.9rem;font-weight:700;border-radius:12px;text-decoration:none;margin-bottom:10px;box-sizing:border-box;text-align:center;">Browse Services</a>
  ...
  <template v-else>
    ...
    <div style="max-width:800px;margin:0 auto;padding:28px 24px 120px;">
      ...
      <div v-if="currentStep === 1">
        <p style="font-size:0.65rem;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:#5c5751;margin:0 0 8px;">Portfolio / Cover Photos <span style="color:#ef4444;">*</span> <span style="color:#9ca3af;font-weight:400;text-transform:none;letter-spacing:0;">(at least one, up to 6)</span></p>
        ...
        >
          ...
          <p style="font-size:0.9rem;font-weight:700;color:#3A3330;margin:0 0 4px;">Show your work</p>
          ...
          <label style="display:inline-flex;align-items:center;gap:8px;padding:10px 20px;background:#ED730C;color:#fff;border-radius:10px;cursor:pointer;font-size:0.845rem;font-weight:700;font-family:'DM Sans',sans-serif;">
            ...
            Upload from device
            <input type="file" multiple accept="image/jpeg,image/png,image/webp" class="sr-only" aria-label="Upload portfolio photos" @change="onFileChange">
          </label>
        </div>

        <div v-if="imagePreviews.length">
          <p style="font-size:0.65rem;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:#5c5751;margin:0 0 10px;">Uploaded Photos <span style="color:#9ca3af;font-weight:400;text-transform:none;letter-spacing:0;">— tap to set the cover</span></p>
          <div style="display:flex;flex-wrap:wrap;gap:10px;">
            <div
              v-for="(src, i) in imagePreviews" :key="i"
              role="button" tabindex="0"
              :aria-label="'Set photo ' + (i + 1) + ' as the cover'"
            ...
            >
              <img :src="src" :alt="'Service photo ' + (i + 1)" style="width:100%;height:100%;object-fit:cover;">
              <div v-if="i === primaryIndex" style="position:absolute;bottom:0;left:0;right:0;background:#ED730C;color:#fff;font-size:0.55rem;font-weight:800;text-align:center;padding:3px;text-transform:uppercase;letter-spacing:.05em;">Cover</div>
      ...
      <div v-else-if="currentStep === 2">
        ...
        <div style="margin-bottom:14px;">
          <label for="sv-cat" style="font-size:0.65rem;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:#5c5751;display:block;margin-bottom:6px;">Category <span style="color:#ef4444;">*</span></label>
          <select id="sv-cat" v-model="category" style="width:100%;padding:11px 13px;border:1.5px solid #EDE8E0;border-radius:11px;font-size:0.9rem;font-family:'DM Sans',sans-serif;outline:none;color:#3A3330;background:#fff;box-sizing:border-box;">
        ...
        <div style="margin-bottom:14px;">
          <label for="sv-title" style="font-size:0.65rem;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:#5c5751;display:block;margin-bottom:6px;">Service Title <span style="color:#ef4444;">*</span></label>
          <input id="sv-title" v-model="title" required placeholder="e.g., Logo & Brand Identity Design" style="width:100%;padding:11px 13px;border:1.5px solid #EDE8E0;border-radius:11px;font-size:0.9rem;font-family:'DM Sans',sans-serif;outline:none;color:#3A3330;box-sizing:border-box;" @focus="e => e.target.style.borderColor='#ED730C'" @blur="e => e.target.style.borderColor='#EDE8E0'">
        </div>

        <!-- Rate + Rate type -->
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-bottom:14px;">
          <div>
            <label for="sv-rate" style="font-size:0.65rem;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:#5c5751;display:block;margin-bottom:6px;">Rate ({{ currency.symbol }}) <span style="color:#9ca3af;font-weight:400;text-transform:none;letter-spacing:0;">(optional)</span></label>
            <input id="sv-rate" v-model="rate" type="number" min="0" inputmode="numeric" placeholder="e.g., 3000" style="width:100%;padding:11px 13px;border:1.5px solid #EDE8E0;border-radius:11px;font-size:0.9rem;font-family:'DM Sans',sans-serif;outline:none;color:#3A3330;box-sizing:border-box;">
          </div>
          <div>
            <label for="sv-ratetype" style="font-size:0.65rem;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:#5c5751;display:block;margin-bottom:6px;">Billed</label>
            <select id="sv-ratetype" v-model="rateType" :disabled="!rate" :style="{ width:'100%', padding:'11px 13px', border:'1.5px solid #EDE8E0', borderRadius:'11px', fontSize:'0.9rem', fontFamily:`'DM Sans',sans-serif`, outline:'none', color: rate ? '#3A3330' : '#9ca3af', background:'#fff', boxSizing:'border-box', cursor: rate ? 'pointer' : 'not-allowed' }">
        ...
        <!-- Delivery method -->
        <div style="margin-bottom:14px;">
          <span style="font-size:0.65rem;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:#5c5751;display:block;margin-bottom:8px;">Delivery Method</span>
          <div role="radiogroup" aria-label="Delivery method" style="display:grid;grid-template-columns:repeat(3,1fr);gap:10px;">
            <button v-for="d in deliveryOptions" :key="d" type="button" role="radio" :aria-checked="delivery === d" @click="delivery = d"
              :style="{ padding:'12px 8px', borderRadius:'12px', textAlign:'center', fontSize:'0.82rem', fontWeight:'700', cursor:'pointer', fontFamily:`'DM Sans',sans-serif`, transition:'all .15s', border: delivery === d ? '2px solid #ED730C' : '1.5px solid #EDE8E0', background: delivery === d ? '#FFF4EC' : '#fff', color: delivery === d ? '#ED730C' : '#6b7280' }">
              {{ d }}
            </button>
          </div>
        </div>
        ...
        <div style="margin-bottom:4px;">
          <label style="font-size:0.65rem;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:#5c5751;display:block;margin-bottom:6px;">Service Area <span style="color:#9ca3af;font-weight:400;text-transform:none;letter-spacing:0;">— click the map to pin (for in-person)</span></label>
          <div id="service-map" style="position:relative;z-index:0;width:100%;height:240px;border-radius:16px;border:1.5px solid #EDE8E0;overflow:hidden;margin-bottom:8px;background:#f3f4f6;"></div>
          <div style="position:relative;">
            <input v-model="location" aria-label="Selected service area" placeholder="Click the map to set a location..." readonly style="width:100%;padding:11px 13px 11px 38px;border:1.5px solid #EDE8E0;border-radius:11px;font-size:0.85rem;font-family:'DM Sans',sans-serif;outline:none;color:#3A3330;background:#FAF8F5;box-sizing:border-box;cursor:default;">
      ...
      <div v-else-if="currentStep === 3">
        ...
        <div style="margin-bottom:18px;">
          <label for="sv-desc" style="font-size:0.65rem;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:#5c5751;display:block;margin-bottom:6px;">Description <span style="color:#9ca3af;font-weight:400;text-transform:none;letter-spacing:0;">(optional)</span></label>
          <textarea id="sv-desc" v-model="description" placeholder="Describe what's included, your process, turnaround time..." rows="3" style="width:100%;padding:11px 13px;border:1.5px solid #EDE8E0;border-radius:11px;font-size:0.9rem;font-family:'DM Sans',sans-serif;outline:none;color:#3A3330;resize:vertical;box-sizing:border-box;" @focus="e => e.target.style.borderColor='#ED730C'" @blur="e => e.target.style.borderColor='#EDE8E0'"></textarea>
        </div>

        <!-- Highlights -->
        <div style="margin-bottom:18px;">
          <span style="font-size:0.65rem;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:#5c5751;display:block;margin-bottom:10px;">Highlights</span>
        ...
        <!-- Swap for -->
        <div style="margin-bottom:4px;">
          <label for="sv-swap" style="font-size:0.65rem;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:#5c5751;display:block;margin-bottom:6px;">Open to swap for <span style="color:#9ca3af;font-weight:400;text-transform:none;letter-spacing:0;">(optional)</span></label>
          <textarea id="sv-swap" v-model="swapFor" placeholder="e.g., Web development, photography, tutoring, or other services..." rows="2" style="width:100%;padding:11px 13px;border:1.5px solid #EDE8E0;border-radius:11px;font-size:0.9rem;font-family:'DM Sans',sans-serif;outline:none;color:#3A3330;resize:vertical;box-sizing:border-box;" @focus="e => e.target.style.borderColor='#ED730C'" @blur="e => e.target.style.borderColor='#EDE8E0'"></textarea>
        </div>
      </div>
    </div>
    ...
    <div style="position:fixed;bottom:0;left:0;right:0;background:#fff;border-top:1px solid #EDE8E0;padding:16px 24px;z-index:110;">
      <div style="max-width:800px;margin:0 auto;display:flex;justify-content:space-between;align-items:center;gap:12px;">
        ...
        <!-- Step 3: Publish -->
        <button v-if="currentStep === 3" @click="publishService" :disabled="loading" style="flex:1;padding:13px 32px;background:#ED730C;color:#fff;font-size:0.9rem;font-weight:700;border:none;border-radius:12px;cursor:pointer;font-family:'DM Sans',sans-serif;display:inline-flex;align-items:center;justify-content:center;gap:8px;">
          <span v-if="!loading">Publish Service</span>

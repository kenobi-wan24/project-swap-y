...
const stepLabels  = ['Add Photos', 'Item Details', 'Listing Preferences']
...
const step2Valid = computed(() => title.value.trim() && category.value && condition.value)
...
async function initMap() {
  ...
  leafletMap = window.L.map('item-map', { center: [12.8797, 121.7740], zoom: 6, zoomControl: true })
...
async function postItem() {
  ...
  try {
    const res = await fetch('/items', { method: 'POST', headers: { 'X-CSRF-TOKEN': csrfToken }, body: formData })
    const data = await res.json()
    if (data.success) { postedItem.value = data.item; posted.value = true }
  } catch (e) { console.error('Post item failed', e) }
...
<div style="min-height:100vh;background:#fff;font-family:'DM Sans',sans-serif;">
  ...
  <div v-if="posted" style="min-height:100vh;display:flex;align-items:center;justify-content:center;padding:24px;">
    <div style="background:#fff;border-radius:24px;padding:48px 32px;max-width:480px;width:100%;text-align:center;box-shadow:0 4px 24px rgba(0,0,0,0.08);border:1px solid #EDE8E0;">
      ...
      <h2 style="font-size:1.5rem;font-weight:800;color:#3A3330;margin:0 0 10px;">Item Posted!</h2>
      <p style="font-size:0.9rem;color:#6b7280;margin:0 0 32px;">
        <strong style="color:#3A3330;">{{ postedItem?.title }}</strong> is now live and ready for matches.
      </p>
  ...
  <template v-else>
    ...
    <div style="max-width:800px;margin:0 auto;padding:28px 24px 120px;">
      ...
      <div v-if="currentStep === 1">
        <p style="font-size:0.65rem;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:#5c5751;margin:0 0 8px;">Item Photos <span style="color:#ef4444;">*</span> <span style="color:#9ca3af;font-weight:400;text-transform:none;letter-spacing:0;">(at least one, up to 5)</span></p>
        ...
        >
          ...
          <label style="display:inline-flex;align-items:center;gap:8px;padding:10px 20px;background:#ED730C;color:#fff;border-radius:10px;cursor:pointer;font-size:0.845rem;font-weight:700;font-family:'DM Sans',sans-serif;">
            ...
            Upload from device
            <input type="file" multiple accept="image/jpeg,image/png,image/webp" class="sr-only" aria-label="Upload item photos" @change="onFileChange">
          </label>
        </div>
        ...
        <div v-if="imagePreviews.length">
          ...
          <div style="display:flex;flex-wrap:wrap;gap:10px;">
            <div
              ...
              :style="{ position: 'relative', width: '88px', height: '88px', borderRadius: '10px', overflow: 'hidden', cursor: 'pointer', flexShrink: '0', border: i === primaryIndex ? '2.5px solid #ED730C' : '2px solid #EDE8E0' }"
            >
              <img :src="src" :alt="'Item photo ' + (i + 1)" style="width:100%;height:100%;object-fit:cover;">
      ...
      <div v-else-if="currentStep === 2">
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-bottom:14px;">
          <div>
            <label for="it-title" style="font-size:0.65rem;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:#5c5751;display:block;margin-bottom:6px;">Item Title <span style="color:#ef4444;">*</span></label>
            <input id="it-title" v-model="title" required placeholder="e.g., Vintage Leica M4" style="width:100%;padding:11px 13px;border:1.5px solid #EDE8E0;border-radius:11px;font-size:0.9rem;font-family:'DM Sans',sans-serif;outline:none;color:#3A3330;box-sizing:border-box;" @focus="e => e.target.style.borderColor='#ED730C'" @blur="e => e.target.style.borderColor='#EDE8E0'">
          </div>
          <div>
            <label for="it-cat" style="font-size:0.65rem;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:#5c5751;display:block;margin-bottom:6px;">Category <span style="color:#ef4444;">*</span></label>
            <select id="it-cat" v-model="category" style="width:100%;padding:11px 13px;border:1.5px solid #EDE8E0;border-radius:11px;font-size:0.9rem;font-family:'DM Sans',sans-serif;outline:none;color:#3A3330;background:#fff;box-sizing:border-box;">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-bottom:14px;">
          <div>
            <label for="it-cond" style="font-size:0.65rem;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:#5c5751;display:block;margin-bottom:6px;">Condition <span style="color:#ef4444;">*</span></label>
            <select id="it-cond" v-model="condition" style="width:100%;padding:11px 13px;border:1.5px solid #EDE8E0;border-radius:11px;font-size:0.9rem;font-family:'DM Sans',sans-serif;outline:none;color:#3A3330;background:#fff;box-sizing:border-box;">
        ...
        <div style="margin-bottom:14px;">
          <label for="it-desc" style="font-size:0.65rem;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:#5c5751;display:block;margin-bottom:6px;">Description <span style="color:#9ca3af;font-weight:400;text-transform:none;letter-spacing:0;">(optional)</span></label>
          <textarea id="it-desc" v-model="description" placeholder="Describe the history, features, and why you're swapping it..." rows="3" style="width:100%;padding:11px 13px;border:1.5px solid #EDE8E0;border-radius:11px;font-size:0.9rem;font-family:'DM Sans',sans-serif;outline:none;color:#3A3330;resize:vertical;box-sizing:border-box;" @focus="e => e.target.style.borderColor='#ED730C'" @blur="e => e.target.style.borderColor='#EDE8E0'"></textarea>
        </div>

        <!-- Location map -->
        <div style="margin-bottom:4px;">
          <label style="font-size:0.65rem;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:#5c5751;display:block;margin-bottom:6px;">Item Location <span style="color:#9ca3af;font-weight:400;text-transform:none;letter-spacing:0;">— click the map to pin</span></label>
          <div id="item-map" style="position:relative;z-index:0;width:100%;height:240px;border-radius:16px;border:1.5px solid #EDE8E0;overflow:hidden;margin-bottom:8px;background:#f3f4f6;"></div>
          <div style="position:relative;">
            <input v-model="location" aria-label="Selected item location" placeholder="Click the map to set a location..." readonly style="width:100%;padding:11px 13px 11px 38px;border:1.5px solid #EDE8E0;border-radius:11px;font-size:0.85rem;font-family:'DM Sans',sans-serif;outline:none;color:#3A3330;background:#FAF8F5;box-sizing:border-box;cursor:default;">
      ...
      <div v-else-if="currentStep === 3">
        <div style="margin-bottom:20px;">
          <label style="font-size:0.65rem;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:#5c5751;display:block;margin-bottom:8px;">Listing Type</label>
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">
            <div style="padding:13px;border-radius:12px;border:2px solid #ED730C;background:#FFF4EC;text-align:center;font-size:0.875rem;font-weight:700;color:#EA580C;">Swap</div>
            <div style="padding:13px;border-radius:12px;border:1.5px solid #EDE8E0;background:#FAF8F5;text-align:center;font-size:0.875rem;font-weight:600;color:#9ca3af;">Sell or swap</div>
          </div>
        </div>

        <div style="margin-bottom:20px;">
          <label for="it-wants" style="font-size:0.65rem;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:#5c5751;display:block;margin-bottom:6px;">What are you looking for?</label>
          <input id="it-wants" v-model="lookingFor" placeholder="e.g., iPhone, sneakers, gadgets" style="width:100%;padding:11px 13px;border:1.5px solid #EDE8E0;border-radius:11px;font-size:0.9rem;font-family:'DM Sans',sans-serif;outline:none;color:#3A3330;box-sizing:border-box;" @focus="e => e.target.style.borderColor='#ED730C'" @blur="e => e.target.style.borderColor='#EDE8E0'">
        </div>

        <div style="margin-bottom:4px;">
          <span style="font-size:0.65rem;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:#5c5751;display:block;margin-bottom:10px;">Swap Conditions</span>
          <div style="display:flex;flex-direction:column;gap:10px;">
            <button v-for="opt in swapConditionOptions" :key="opt.value" type="button" role="checkbox" :aria-checked="swapConditions.includes(opt.value)" @click="toggleCondition(opt.value)"
              style="display:flex;align-items:center;gap:10px;cursor:pointer;background:none;border:none;padding:0;width:100%;text-align:left;font-family:'DM Sans',sans-serif;">
              <span :style="{ width: '18px', height: '18px', borderRadius: '5px', flexShrink: '0', border: swapConditions.includes(opt.value) ? '2px solid #ED730C' : '2px solid #EDE8E0', background: swapConditions.includes(opt.value) ? '#ED730C' : '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all .15s' }">
                <svg v-if="swapConditions.includes(opt.value)" width="10" height="10" fill="none" stroke="#fff" stroke-width="3" viewBox="0 0 24 24" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" d="M20 6L9 17l-5-5"/></svg>
              </span>
    ...
    <div style="position:fixed;bottom:0;left:0;right:0;background:#fff;border-top:1px solid #EDE8E0;padding:16px 24px;z-index:110;">
      <div style="max-width:800px;margin:0 auto;display:flex;justify-content:space-between;align-items:center;gap:12px;">
        ...
        <!-- Step 3: Post -->
        <button v-if="currentStep === 3" @click="postItem" :disabled="loading" style="flex:1;padding:13px 32px;background:#ED730C;color:#fff;font-size:0.9rem;font-weight:700;border:none;border-radius:12px;cursor:pointer;font-family:'DM Sans',sans-serif;display:inline-flex;align-items:center;justify-content:center;gap:8px;">
